...
- Avoid moving root entry files (`server.ts`, `worker.ts`, `cli.ts`) into `src/` unless all scripts and docs are updated together.
...
- Worker queue processing is initialized in `worker.ts` and uses `RabbitMqServiceImpl`.
...
| Root entry files `server.ts`, `worker.ts`, `cli.ts` | `package.json` scripts, `Dockerfile`, `AGENTS.md` commands section | Runtime entry points are operational interfaces used by scripts and docs. |
