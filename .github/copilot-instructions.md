...
- Avoid moving root entry files (`server.ts`) into `src/` unless all scripts and docs are updated together.
...
- API is resilient to DB startup failures; preserve this behavior unless explicitly changing reliability semantics.
...
| `src/api/routes/saldos.ts` | `src/api/plugins/auth.ts`, `src/api/services/JobService.ts`, `__tests__/api/JobService.test.ts` (and add route tests if needed) | Endpoint behavior and auth assumptions are coupled. |
...
| `src/api/routes/health.ts` | `src/infrastructure/persistence/PrismaService.ts`, `__tests__/api/routes.test.ts` | Health contracts should reflect real runtime telemetry from DB integrations. |
| `src/api/config.ts` or `config.json` | `src/api/server.ts`, `k8s/configmap.yaml`, `k8s/secret.yaml` | Config keys are shared across API and deployment manifests. |
...
| Root entry file `server.ts` | `package.json` scripts, `Dockerfile`, `AGENTS.md` commands section | Runtime entry points are operational interfaces used by scripts and docs. |
...
- If config keys changed, verify API + k8s manifests still agree.
